...
- **Sortable table**: `DataTable` inside the app's table frame `<div className="overflow-hidden rounded-[11px] border border-[#1E344C] bg-card">`; columns put alignment in `meta` (`text-right` for numeric, `pl-4` first / `pr-4` last, `w-full` on the name column), headings are authored UPPERCASE, and `TableCell` already renders mono. Dark-committed chrome — use under a `class="dark"` root. Static tables compose the `Table` parts directly with the same classes.
